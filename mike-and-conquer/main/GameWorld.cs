...
// Still happens when all above are commented out


            terrainMap.Add(2544, "Content\\TC02.tem");   // This is the one

            Pickup here
            Shadow renders correctly if only draw above tree
            If also draw one below, shadow gets messed up
            Debug and determine why
            May need to revisit exact pixel that is not getting mapped right...
                // Believe the issue is that, the same texture is being used for both instnaces of the tree
                // So that shadows get mapped for the first instance of the tree
                // but then those shadow pixels in the texture get overwritten when the shadows get mapped for the
                // second instance of the tree

            terrainMap.Add(3052, "Content\\TC02.tem");


            // Still happens when all below are commented out
